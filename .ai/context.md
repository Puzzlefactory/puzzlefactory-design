...
- Theme Authoring is now an active workstream with a usable `apps/theme-author` React + Vite + React Router app. It is a separate designer-facing app/workflow from Kitchen Sink. It consumes `@puzzlefactory/color-engine` for curated presets, normalized input validation, generated semantic CSS injection, editable custom color roles, editable header/sidebar/footer role-treatment mappings, designer-facing light/dark/high-contrast preview frames, region APCA review, generated CSS artifact/manifest inspection, and human-readable APCA readiness diagnostics. Kitchen Sink remains the diagnostic lab. Color.js is approved for future Theme Authoring picker/converter/dev tooling when needed, but not for the core color-engine runtime by default.
