...
- Theme Authoring is now an active workstream with a minimal `apps/theme-author` React + Vite + React Router app scaffold. It is a separate designer-facing app/workflow from Kitchen Sink. Kitchen Sink remains the diagnostic lab; Theme Authoring is intended to guide normalized theme input editing, light/dark/high-contrast preview, human-readable APCA review, and CSS artifact/manifest export.
