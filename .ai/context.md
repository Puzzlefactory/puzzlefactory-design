...
- `packages/color-engine` is now the v2 `@puzzlefactory/color-engine` workspace package; current v2 output includes neutral/surface generation with theme-specific surface separation presets, dedicated foreground/text generation, chrome/border generation, explicit light and dark seed primary usage ramps, explicit light and dark seed status usage ramps, per-family seed policy, stable semantic aliases, load-order-ready structured CSS output, static CSS artifact metadata/export support, APCA calculation exports, state-aware diagnostic APCA assertion reports over generated surface interaction states, a public caller-ordered contrast foreground resolver that can check multiple backgrounds, balanced dark solid tuning, dedicated foreground-based solid text resolution, named soft-surface text treatment strategies, a representative preset/seed assertion regression matrix, curated example theme presets with varied built-in status palettes, generated custom color role families plus `--ds-role-*` semantic aliases, deterministic custom-role ordering, and custom role diagnostic assertion pairs
...
- `packages/themes` is now the real `@puzzlefactory/themes` workspace package. It defines the versioned canonical theme source, theme identity and region mapping validation, normalized color-engine input, resolved region semantics, APCA region diagnostics, deterministic CSS/bundle/manifest artifacts, and explicit theme/color-engine/release metadata above `@puzzlefactory/color-engine`.
...
- Theme Authoring is now an active workstream with a usable `apps/theme-author` React + Vite + React Router app. It is a separate designer-facing app/workflow from Kitchen Sink. It consumes `@puzzlefactory/color-engine` for curated inputs and `@puzzlefactory/themes` for canonical source composition, resolved regions, APCA region diagnostics, and deterministic artifacts. It supports editable custom roles and header/sidebar/footer role-treatment mappings, light/dark/high-contrast previews, human-readable readiness diagnostics, exact artifact/manifest inspection, tenant/theme identity, browser-local draft save/load with optimistic revisions, and immutable local publication versions. The replaceable async persistence port is hardened against malformed/tampered storage, revision rollover, and stale cross-identity results; its development-only browser adapter stores canonical manifests/artifacts without duplicated engine composition. Kitchen Sink remains the diagnostic lab. Color.js is approved for future Theme Authoring picker/converter/dev tooling when needed, but not for the core color-engine runtime by default.
...
packages/
  ...
  themes/        // @puzzlefactory/themes; canonical theme composition, region diagnostics, and deterministic artifact manifests
...
Color engine v1 is closed as reference. Its package boundaries, token CSS output, and prior kitchen-sink visualization are useful, but its broad generic ramp generation model is not the future path. Color engine v2 has compact neutral/surface/chrome, foreground/text, primary, status, and custom-role usage generation; fixed high-contrast themes; stable semantic aliases; ordered static CSS artifacts; APCA calculation plus state-aware assertion reports; caller-ordered contrast foreground resolution; named text treatments; curated presets; deterministic custom-role output; and immediate Kitchen Sink diagnostics. Theme Authoring has editable custom roles and region role-treatment mappings, per-theme contrast-resolved subordinate labels, human-readable diagnostics, canonical artifact inspection, and a hardened replaceable repository port for local draft/publication workflow verification. `@puzzlefactory/themes` owns canonical source composition and deterministic artifacts. Repository lint/build/test/typecheck, dependency audit, Chromium app tests, browser interaction/visual review, and independent hardening re-review passed on 2026-07-16. Consumer-owned tenant databases, Azure Blob/CDN upload, activation/rollback/caching/bootstrap, component tone APIs, independent border authoring, a v2 token package migration, and broad component library work remain deferred.
...
4. Read `.ai/workstreams/tenant-theme-publishing.md` before theme composition, persistence, or consumer-pilot work.
5. Check git status.
6. Continue from the workstream's Next Actions rather than relying on chat history.
