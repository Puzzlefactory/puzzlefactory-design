...
- `packages/color-engine` is now the v2 `@puzzlefactory/color-engine` workspace package; CE2-01 implements only neutral and surface generation
...
```
packages/
  color-engine/    // @puzzlefactory/color-engine; v2 neutral/surface implementation
...
apps/
  ...
  kitchen-sink/       // React + Vite + React Router 7 verification shell wired to v2 neutral/surface output
```
...
Color engine v1 is closed as reference. Its package boundaries, token CSS output, and prior kitchen-sink visualization are useful, but its broad generic ramp generation model is not the future path. Color engine v2 has started with neutral and surface generation only: separate neutral, light-surface, and dark-surface seeds; compact light/dark surface ramps; named presets; and immediate kitchen-sink visualization. Component layer work remains deferred.
