...
- `packages/components` is now a real `@puzzlefactory/components` workspace package containing a minimal Web Component proof (`pf-button` and `pf-alert`) that consumes v2 semantic CSS custom properties only
