...
- Runtime stack: React 19, Vite 8, React Router 7.
- Current behavior: private workspace app with routes for Overview, Theme Input, Preview, Regions, Artifacts, Diagnostics, and Publishing. It imports `@puzzlefactory/color-engine` for curated preset/editor types and `@puzzlefactory/themes` for canonical theme source composition, resolved region semantics, APCA region diagnostics, and deterministic artifact bundles/manifests. It edits normalized seed/policy/surface/text input fields, creates/edits/enables/removes custom roles, injects generated semantic CSS variables, renders designer-facing app-shell preview frames for all four themes, lets header/sidebar/footer select an enabled authored role and complete `soft` or `solid` treatment, resolves subordinate region labels to the quietest APCA-safe text token across rest/hover backgrounds, and translates color plus region diagnostics into readiness guidance. A replaceable async repository port has a browser-local development adapter for tenant-scoped drafts, optimistic revisions, exact-bundle preview/publication, immutable versions, and visible failures that preserve the active draft. Region borders remain derived from the selected complete treatment rather than independently authored.
