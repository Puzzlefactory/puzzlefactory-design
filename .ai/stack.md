...
- Supported Node.js versions: `^20.19.0 || ^22.13.0 || >=24`.
- Root lint: ESLint 10 with TypeScript ESLint; `npm run lint` checks the repository.
- Repository CI: GitHub Actions runs install, lint, build, full tests (including Playwright Chromium), typecheck, and high-severity dependency audit on pushes and pull requests.
...
Engine-created APCA reports cover `text-primary`, `text-secondary`, and `text-muted` on generated rest, hover, selected, and pressed surface states using state values shared with CSS generation. The public assertion helper preserves its legacy report unless resolved surface presets are supplied. Custom roles use locale-independent code-unit normalization order so canonical artifacts remain deterministic across host locales.
...
- Current consumers: Theme Author uses this package for canonical source composition, region semantics, region APCA diagnostics, and exact artifact/manifest previews
...
- Current public component API:
  - `pf-button`: `disabled` attribute/property, `variant` attribute/property with `primary` or `secondary`, forwarded `aria-label`, `focus(options?)`, and `click()`. Programmatic `click()` delegates to the internal button only when enabled. Form participation, submit/reset behavior, `type`, `name`, `value`, `form`, and constraint validation are deferred.
...
- Current behavior: routed verification tool that consumes v2 `@puzzlefactory/color-engine`, injects generated CSS directly, switches `data-theme-v2`, consumes `@puzzlefactory/react-components` wrappers over `@puzzlefactory/components`, and displays curated theme preset controls, shared/light/dark surface separation controls with inherited shared fallback support, neutral/surface/chrome/text controls, separate light/dark primary and status seed controls, editable custom role example controls, seed policies, text treatments, primitive/semantic/custom-role output, CSS/token inspection, usage previews, component proofs, and an APCA assertion report grouped by built-in theme/role plus custom role groups. Current Kitchen Sink defaults use `evergreen` plus `pending`, `promo`, and `billing`, producing 356/356 passing assertion pairs after state-aware surface coverage. When anchored policy is active, linked failures are identified as seed-preservation tradeoffs.
...
Status: editable custom-role authoring, canonical region composition, preview, diagnostics, artifacts, and local draft/publication workflow implemented.
...
- Runtime stack: React 19, Vite 8, React Router 7.
- Current behavior: private workspace app with routes for Overview, Theme Input, Preview, Regions, Artifacts, Diagnostics, and Publishing. It imports `@puzzlefactory/color-engine` for curated preset/editor types and `@puzzlefactory/themes` for canonical theme source composition, resolved region semantics, APCA region diagnostics, and deterministic artifact bundles/manifests. It edits normalized seed/policy/surface/text input fields, creates/edits/enables/removes custom roles, renders all four themes, lets header/sidebar/footer select complete role treatments, resolves subordinate labels to the quietest APCA-safe token across rest/hover, and translates diagnostics into readiness guidance. Its replaceable async repository port has a development-only browser adapter with deep storage validation, optimistic revisions, immutable compact publication records, canonical recomposition/tamper checks, safe integer boundaries, committed-identity async guards, and visible failure preservation. Region borders remain derived from the selected complete treatment rather than independently authored.
- Expected package inputs: `@puzzlefactory/color-engine` for curated color inputs; `@puzzlefactory/themes` for canonical composition and artifacts; future `@puzzlefactory/tokens` for a broader non-color token model.
