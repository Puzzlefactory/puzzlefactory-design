...
CE2-20C is implemented. Custom roles now add APCA diagnostic assertion pairs when configured. Each custom role contributes ten required diagnostic pairs: light and dark soft text on soft rest/hover backgrounds, plus light and dark solid text on solid rest/hover/pressed backgrounds. These pairs use the existing `status-soft` and `status-solid` thresholds and keep the assertion ids/semantic names under `role-{id}-*`. Built-in assertion output remains unchanged when `customRoles` is omitted. Component tone APIs, enforcement, and auto-tuning remain deferred.

CE2-20D is implemented. Kitchen Sink now visualizes custom color roles from the existing generated `customRoles` input. The Controls view includes editable example roles for `pending`, `promo`, and `billing` with enable toggles, light/dark seeds, and seed policy selection. The app passes enabled roles into `createColorEngineTheme`, rejects duplicate enabled role ids in the app error surface instead of silently collapsing them, and generates unique `custom-n` ids when adding rows. The Primitives view dynamically renders custom role primitive families. The Semantic view renders custom role alias swatches for light and dark themes. The Themes view renders custom role soft and solid cards in each theme boundary. The Assertions view separates built-in assertion groups from custom role assertion groups and matches custom role assertions by exact generated aliases so prefix ids such as `foo` and `foo-bar` cannot be conflated. The default Kitchen Sink examples produce 106/106 passing APCA pairs. No component tone APIs, tenant storage, enforcement, auto-tuning, new built-in roles, runtime dependencies, or high-contrast output were added.
...
CE2-31 is implemented. Engine-created APCA reports now cover `text-primary`, `text-secondary`, and `text-muted` on every generated rest, hover, selected, and pressed surface level for light and dark themes, using state values shared with CSS generation so diagnostics cannot drift from emitted tokens. The public `createContrastAssertionReport(...)` helper keeps its legacy 152-pair behavior when surface presets are omitted; state-aware callers pass resolved presets and receive the expanded 296-pair built-in report. Custom roles are normalized in locale-independent code-unit order so canonical theme artifacts are deterministic across host locales. Focused assertion, ordering, and compatibility tests pass, and `@puzzlefactory/color-engine` retains zero runtime dependencies.

## Next Actions

- Pause for direction selection. Theme Authoring and the portable theme composition/publishing foundation are complete; the next larger path can be a v2 token package migration, a high-contrast forced-colors follow-up, a consumer-owned tenant/blob pilot, or a Lit form-control prototype.
