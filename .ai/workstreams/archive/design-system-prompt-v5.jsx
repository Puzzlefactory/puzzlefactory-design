...
const REVIEW_SYSTEM = `You are a senior principal engineer and design systems architect with deep expertise in color science, token architecture, accessibility standards, and frontend system design.

You are reviewing revision 5 of a technical prompt for a design system color engine and token architecture. This has gone through four prior revision cycles. Your job is to find anything that remains genuinely wrong, incomplete, or ambiguous.

Be direct. Reference exact sections. Do not pad with praise. If something is correct and complete, skip it.
...
## Ambiguities
## Minor Issues
## Assessment — is this ready for an implementation agent?`;
...
export default function App() {
  ...
  const runReview = async () => {
    setRunning(true); setTab("review"); setReview(null);
    try {
      ...
      setReview(d.content?.filter(b => b.type==="text").map(b=>b.text).join("\n") || "No response.");
    } catch(e) { setReview("Failed: " + e.message); }
    setRunning(false);
  };

  const md = (text) => {
    let inCode = false, acc = [], out = [];
    text.split("\n").forEach((line, i) => {
      if (line.startsWith("```")) {
        if (!inCode) { inCode=true; acc=[]; }
        else {
          inCode=false;
          out.push(<pre key={i} style={{background:"#0d1117",border:"1px solid #21262d",borderRadius:"6px",padding:"14px 16px",fontSize:"11px",color:"#a5d6ff",overflowX:"auto",margin:"10px 0",lineHeight:1.6,whiteSpace:"pre"}}>{acc.join("\n")}</pre>);
        }
        return;
      }
      if (inCode) { acc.push(line); return; }

      const tok = (t) => {
        const parts = t.split(/(`[^`]+`|\*\*[^*]+\*\*)/g);
        return parts.map((p,j) => {
          if (p.startsWith("`") && p.endsWith("`")) return <code key={j} style={{background:"#1c2128",border:"1px solid #30363d",borderRadius:"3px",padding:"1px 5px",fontSize:"11px",color:"#a5d6ff",fontFamily:"inherit"}}>{p.slice(1,-1)}</code>;
          if (p.startsWith("**") && p.endsWith("**")) return <strong key={j} style={{color:"#e2e8f0",fontWeight:600}}>{p.slice(2,-2)}</strong>;
          return p;
        });
      };

      if (line.startsWith("# ")) { out.push(<h1 key={i} style={{fontSize:"16px",fontWeight:700,color:"#f7fafc",marginBottom:"4px",marginTop:"8px"}}>{line.slice(2)}</h1>); return; }
      if (line.startsWith("## ")) { out.push(<h2 key={i} style={{fontSize:"11px",fontWeight:700,color:"#e2e8f0",marginTop:"26px",marginBottom:"8px",textTransform:"uppercase",letterSpacing:"0.1em",borderBottom:"1px solid #21262d",paddingBottom:"5px"}}>{line.slice(3)}</h2>); return; }
      if (line.startsWith("### ")) { out.push(<h3 key={i} style={{fontSize:"11px",fontWeight:600,color:"#79c0ff",marginTop:"16px",marginBottom:"5px",textTransform:"uppercase",letterSpacing:"0.08em"}}>{line.slice(4)}</h3>); return; }
      if (line.match(/^\|/)) {
        const cells = line.split("|").filter(c=>c.trim()!=="");
        if (cells.every(c=>c.trim().match(/^-+$/))) return;
        out.push(<div key={i} style={{display:"flex",borderBottom:"1px solid #1c2128",padding:"3px 0"}}>{cells.map((c,j)=><div key={j} style={{flex:1,fontSize:"12px",color:"#cbd5e0",padding:"2px 8px"}}>{c.trim()}</div>)}</div>);
        return;
      }
      if (line.startsWith("- ")||line.startsWith("* ")) { out.push(<li key={i} style={{fontSize:"13px",color:"#cbd5e0",lineHeight:1.75,marginLeft:"20px",listStyleType:"disc",marginBottom:"2px"}}>{tok(line.slice(2))}</li>); return; }
      if (line.trim()==="---") { out.push(<hr key={i} style={{border:"none",borderTop:"1px solid #21262d",margin:"20px 0"}}/>); return; }
      if (line.trim()==="") { out.push(<div key={i} style={{height:"4px"}}/>); return; }
      out.push(<p key={i} style={{fontSize:"13px",color:"#cbd5e0",lineHeight:1.75,marginBottom:"3px"}}>{tok(line)}</p>);
    });
    return out;
  };
  ...
  const Btn = ({label, onClick, active, disabled}) => (
    <button onClick={onClick} disabled={disabled} style={{
      background:active?"#1a472a":disabled?"#161b22":"#21262d",
      border:`1px solid ${active?"#2ea043":"#30363d"}`,
      color:active?"#3fb950":disabled?"#4a5568":"#8b949e",
  ...
  return (
    <div style={{fontFamily:"'JetBrains Mono','Fira Code',monospace",background:"#0d1117",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      <div style={{background:"#161b22",borderBottom:"1px solid #21262d",padding:"16px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px",flexWrap:"wrap"}}>
        <div>
          <div style={{fontSize:"10px",color:"#8b949e",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:"2px"}}>Design System Foundation</div>
          <div style={{fontSize:"15px",fontWeight:700,color:"#f0f6fc"}}>Color Engine & Token Architecture</div>
          <div style={{display:"flex",alignItems:"center",gap:"8px",marginTop:"4px"}}>
            <span style={{fontSize:"10px",background:"#1f3a1f",border:"1px solid #2ea043",color:"#3fb950",padding:"1px 8px",borderRadius:"20px"}}>REVISION 5</span>
            <span style={{fontSize:"10px",color:"#6e7681"}}>Full review pass — all v4 issues addressed</span>
          </div>
        </div>
        <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
          <Btn label={cp?"✓ Copied":"Copy Prompt"} onClick={()=>copyText(INITIAL_PROMPT,setCp)} active={cp} />
          <Btn label={running?"Reviewing...":"Run Sub-Agent Review"} onClick={runReview} disabled={running} />
        </div>
      </div>

      <div style={{background:"#161b22",borderBottom:"1px solid #21262d",padding:"0 24px",display:"flex"}}>
        {["prompt","review"].map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{
            background:"transparent",border:"none",
            borderBottom:tab===t?"2px solid #f78166":"2px solid transparent",
            color:tab===t?"#f0f6fc":"#8b949e",padding:"10px 16px",
            fontSize:"11px",cursor:"pointer",fontFamily:"inherit",
            textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:tab===t?600:400
          }}>{t==="prompt"?"Prompt":"Sub-Agent Review"+(running?" ⟳":"")}</button>
        ))}
      </div>

      <div style={{flex:1,overflow:"auto",padding:"24px"}}>
        {tab==="prompt"&&(
          <div style={{maxWidth:"820px",margin:"0 auto",background:"#161b22",border:"1px solid #21262d",borderRadius:"8px",padding:"36px"}}>
            {md(INITIAL_PROMPT)}
          </div>
        )}
        {tab==="review"&&(
          <div style={{maxWidth:"820px",margin:"0 auto"}}>
            {running&&(
              <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:"8px",padding:"56px",textAlign:"center",color:"#8b949e"}}>
                <div style={{width:"24px",height:"24px",border:"2px solid #30363d",borderTopColor:"#58a6ff",borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 16px"}}/>
                <div style={{fontSize:"13px"}}>Sub-agent reviewing revision 5...</div>
            ...
            {!running&&!review&&(
              <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:"8px",padding:"56px",textAlign:"center",color:"#8b949e"}}>
                ...
                <div style={{fontSize:"13px"}}>Press Run Sub-Agent Review to critique revision 5</div>
              </div>
            )}
            {review&&(
              <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:"8px",padding:"36px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"24px",paddingBottom:"16px",borderBottom:"1px solid #21262d"}}>
                  <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                    <div style={{width:"8px",height:"8px",borderRadius:"50%",background:"#3fb950"}}/>
                    <span style={{fontSize:"10px",color:"#8b949e",textTransform:"uppercase",letterSpacing:"0.1em"}}>Sub-Agent Review — Revision 5</span>
                  </div>
                  <Btn label={cr?"✓ Copied":"Copy Review"} onClick={()=>copyText(review,setCr)} active={cr} />
                </div>
                {md(review)}
              </div>
            )}
      ...
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}::-webkit-scrollbar{width:6px}::-webkit-scrollbar-track{background:#0d1117}::-webkit-scrollbar-thumb{background:#30363d;border-radius:3px}`}</style>
    </div>
  );
}
